...
if main_window.exists():
    ...
    try:
        ...
        tables_menu_item = context_menu.child_window(title="Graphs", control_type="MenuItem")
        tables_menu_item.click_input()
        print("Se seleccionó el menú 'Graphs'.")
        ...
        app_top_window = app.window(best_match="Graphs")  # Reemplaza "Graphs" si es necesario
        app_top_window.print_control_identifiers()

 
        time.sleep(1)
        # Expandir el submenú "BET"
        HK_menu_item = app_top_window.child_window(title="HK method", control_type="MenuItem")
        HK_menu_item.click_input()
        print("Se seleccionó 'HK method' exitosamente.")
        app_top_window = app.window(best_match="HK method")  # Reemplaza "HK method" si es necesario
        app_top_window.print_control_identifiers()
        
        time.sleep(1)

        # Seleccionar " Volume"
        Volume_menu_item = app_top_window.child_window(title=" Volume", control_type="MenuItem")
        Volume_menu_item.click_input()
        print("Se seleccionó ' Volume' exitosamente.")
        app_top_window = app.window(best_match=" Volume")  # Reemplaza "Volume" si es necesario
        app_top_window.print_control_identifiers()
    ...
    print(f"La ventana '.*Quantachrome™ NovaWin - [graph:Isotherm :   Linear: {file_to_open_nameonly}].* no existe!")

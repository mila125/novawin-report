...
def exportar_reporte(main_window, ruta_exportacion, app):
    try:
        ...
        bet_menu_item = app.window(best_match="Tables").child_window(title="Fractal Dimension Methods", control_type="MenuItem")
        bet_menu_item.click_input()
        print("Submenú 'Fractal Dimension Methods' seleccionado.")

        bet_menu_item = app.window(best_match="FFH Method Fractal Dimension(Absorption)")
        bet_menu_item.print_control_identifiers(depth=2)
        single_point_menu_item = bet_menu_item.child_window(title="FFH Method Fractal Dimension(Absorption)", control_type="MenuItem")
        single_point_menu_item.click_input()
        print("Se seleccionó 'FFH Method Fractal Dimension(Absorption)' exitosamente.")

        time.sleep(2)
        secondary_window2 = app.window(title_re=f".*tab: FFH Method Fractal Dimension(Absorption): file_to_open_nameonly.*")
...
def ffhagraph_main(path_qps, path_csv, path_novawin):

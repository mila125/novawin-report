...
try:
    ...
    if 'Radius of curvature' in datos.columns and 'Vapor-Liquid Intrface Area' in datos.columns:
        ...
        # Crear la gráfica para Desorción
        plt.figure(figsize=(8, 6))
        plt.plot(radius_of_curvature, vapor_liquid_interface_area, marker='s', linestyle='-', color='blue', label='Desorption - NK')

        # Configurar etiquetas, título y leyenda
        plt.xlabel('Radius of Curvature')
        plt.ylabel('Vapor-Liquid Interface Area')
        plt.title('Desorption - NK Method')
        plt.grid(True)
        plt.legend()

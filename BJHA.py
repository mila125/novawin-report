...
def exportar_reporte(main_window, ruta_exportacion, app):
    try:
        ...
        bet_menu_item = app.window(best_match="BJH Pore Size Distribution")
        bet_menu_item.print_control_identifiers(depth=2)
        single_point_menu_item = bet_menu_item.child_window(title=" Adsorption ", control_type="MenuItem")
        single_point_menu_item.click_input()
        print("Se seleccionó ' Adsorption ' exitosamente.")

        time.sleep(2)
        secondary_window2 = app.window(title_re=f".*tab:BJH Pore Size Distribution Adsorption : file_to_open_nameonly.*")
...
def bjha_main(path_qps, path_csv, path_novawin):

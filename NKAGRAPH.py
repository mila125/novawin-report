...
def exportar_reporte(main_window, ruta_exportacion, app):
    try:
        ...
        bet_menu_item = app.window(best_match="NK Method Fractal Dimension(Absorption)")
        bet_menu_item.print_control_identifiers(depth=2)
        single_point_menu_item = bet_menu_item.child_window(title="NK Method Fractal Dimension(Absorption)", control_type="MenuItem")
        single_point_menu_item.click_input()
        print("Se seleccionó 'FFH Method Fractal Dimension(Absorption)' exitosamente.")

        time.sleep(2)
        secondary_window2 = app.window(title_re=f".*tab: NK Method Fractal Dimension(Absorption): file_to_open_nameonly.*")
